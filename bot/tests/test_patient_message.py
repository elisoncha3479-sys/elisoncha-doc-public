"""Сообщение пациента в чат → разбор главврачом → память специалистов
...
- clinical → run_multi_agent → write_perdoc (с меткой провенанса
  «со слов пациента») → refresh профилей → reconcile-hook;
- сообщение НЕ от пациентки (владелец/оператор) в клинику не идёт;
- ответ пациенту генерируется тёплым голосом, без назначений лечения.
"""
import asyncio
import sys
