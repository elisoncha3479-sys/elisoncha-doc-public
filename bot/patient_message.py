"""
bot/patient_message.py — свободное сообщение мамы в чат как клинический вход.

Письма и сводка лекарств заканчиваются вопросами к маме. Раньше её ответ
...
2. Если есть клинические сведения — переиспользуем существующий
   ...
   «со слов мамы», обновляем профили и дёргаем reconcile-hook.
   Применяется автономно, без подтверждения владельца (решение владельца).
3. Маме отвечаем тёплым голосом «Команда „Elisoncha Doc“» — без
...
CHAT_PROVENANCE_SOURCE = (
    "Сообщение мамы в чате (со слов пациентки, не подтверждено документом)"
)
...
def classify_patient_message(client, text: str) -> dict:
    ...
    ответить маме, в память ничего не писать."""
    try:
...
def generate_patient_reply(client, text: str, brief: str = "") -> str:
    """Тёплый короткий ответ маме. brief — краткая выжимка того, что
...
def is_patient_author(msg) -> bool:
    """True если сообщение писала пациентка (мама).

    Если задан env PATIENT_TG_USER_ID — строгая сверка по Telegram-id.
    Если не задан — фолбэк v1: считаем автором пациентку (мама пишет
...
async def _ingest_clinical(client, app, text: str) -> str:
    ...
    выжимку заключений специалистов (для ответа маме). Failsafe —
